-- ============================================================================
-- 0018: Every action always raises mood by 0.2 (clamped to the pair ceiling),
-- with NO cooldown, so it's visibly responsive. Meter fills also lost their
-- hourly hard-gate (they keep the first-of-day-full / same-day-repeat-half
-- amounts). APPLIED LIVE (mvyktmxkhwigrrftsbpn) 2026-07-21 as migration
-- `mood_every_action_no_cooldown_0018`. Supersedes 0017's apply_care.
-- Client mirror: mobile/src/app/(tabs)/index.tsx sets all action locks to 0.
-- ============================================================================
...
declare
  ...
  v_last_feed timestamptz; v_last_water timestamptz; v_last_pet timestamptz; v_ceil numeric;
begin
  ...
  -- flat +0.2 mood on EVERY action (no cooldown), clamped to the pair ceiling
